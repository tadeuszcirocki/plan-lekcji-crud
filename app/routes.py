...
def addZajecia():
    form = ZajecieForm()
    if form.is_submitted():
        prowadzacy = db.session.query(Prowadzacy).filter_by(nazwisko=form.prowadzacy.data).one()  #prowadzacy wybierany wg nazwiska ale nizej dodawany do db jest caly obiekt
        przedmiot = db.session.query(Przedmiot).filter_by(nazwa=form.przedmiot.data).one()  #przedmiot wg nazwy
        sala = db.session.query(Sala).filter_by(nr=form.sala.data).one()  #sala wg numeru
        zajecia = Zajecie(prowadzacy=prowadzacy, przedmiot=przedmiot, sala=sala, dzien=form.dzien.data, godzina=form.godzina.data)
